    ...
    if (this.state.OSInfo === {}) {
      ...
      return (
        <div style={{ fontSize: "1.6vw" }}>
          <table>
            <tbody>
              {Object.keys(this.state.OSInfo).map(key => (
                <tr key={this.state.OSInfo[key]["name"]}>
                  <td>{this.state.OSInfo[key]["name"]}</td>
                  <td>{this.state.OSInfo[key]["pct"].toFixed(1)}%</td>
                  <td>{this.state.OSInfo[key]["count"]}</td>
                </tr>
              ))}
            </tbody>
          </table>
