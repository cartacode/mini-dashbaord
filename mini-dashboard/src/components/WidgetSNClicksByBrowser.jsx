    ...
    if (this.state.OSDict === {}) {
      ...
      return (
        <div style={{ fontSize: "1.6vw" }}>
          <table>
            <tbody>
              {Object.keys(this.state.browserInfo).map(key => (
                <tr key={this.state.browserInfo[key]["name"]}>
                  <td>{this.state.browserInfo[key]["name"]}</td>
                  <td>{this.state.browserInfo[key]["pct"].toFixed(1)}%</td>
                  <td>{this.state.browserInfo[key]["count"]}</td>
                </tr>
              ))}
            </tbody>
          </table>
